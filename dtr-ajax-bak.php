...
$sql = "SELECT dtr_employee, (select concat(employee_firstname, ' ', employee_middlename, ' ', employee_lastname) from employees where employee_fid = dtr_employee) full_name, (select dept_name from departments where dept_id = employee_dept) department, employee_dept, employee_scheme, (select scheme_name from schemes where scheme_id = employee_scheme) employee_scheme_name FROM dtr $join_employee $filter ORDER BY dtr_employee LIMIT 1";
db_connect();
$rs = $db_con->query($sql);
$rc = $rs->num_rows;
if ($rc) {
...
$signatory_title = "";
$sql = "SELECT ifnull((select concat(employee_firstname, ' ', employee_middlename, ' ', employee_lastname) from employees where employee_id = dept_head),'') department_signatory, (SELECT employee_position FROM employees WHERE employee_id = dept_head) signatory_title FROM departments WHERE dept_id = $employee_dept";
db_connect();
$rs = $db_con->query($sql);
$rc = $rs->num_rows;
if ($rc) {
...
$halfd = array("$sday"=>0);
$dleave = array("$sday"=>0);
$leve_note = array("$sday"=>"");
$sql = "SELECT option_id, option_fid, option_date, halfday, dtr_leave, leave_note FROM dtr_options WHERE option_fid = '$feid' AND option_date = '$sday'";
db_connect();
$rs = $db_con->query($sql);
$rc = $rs->num_rows;
if ($rc) {
...
// if (($smor_in != "") || ($smor_out != "") || ($saft_in != "") || ($saft_out != "") || ($ignored_logs > 0)) $str_response .= '<td style="text-align: left;"><a href="javascript: dateCon(\'' . $hfid . '\',\'' . date("Y-m-d",strtotime($sday)) . '\');" data-toggle="tooltip" data-placement="right" title="Manage Time Logs/Notes">' . date("j (D)",strtotime($sday)) . '</a></td>';
// else $str_response .= '<td style="text-align: left;">' . date("j (D)",strtotime($sday)) . '</td>';
